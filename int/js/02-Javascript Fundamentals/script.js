...
// descending
const accmapUnit = mapUnits.sort(function (a, b) {
  return b - a;
});
...
const mostExpensive = items.map((item) => {
  return item.price;
});
console.log(mostExpensive);
const maxPrice = Math.max(...mostExpensive);
console.log(maxPrice);

const mutate = 12345;
// to convert to string in array
const iterate = mutate.toString().split("");
console.log(iterate);

// to sum
const iterate2 = iterate.reduce((curr, val) => {
  return Number(curr) + Number(val);
}, 0);
console.log(iterate2);

//to revert
const mutate2 = [1, 2, 3, 4, 5];
const iterate3 = mutate2.join("");
console.log(iterate3); // 12345
...
const decnd = ascend.sort(function (a, b) {
  return a - b;
});
...
if (
  num1.indexOf(1) === num2.indexOf(1) &&
  num1.indexOf(9) === num2.indexOf(9)
) {
  array.push(num1.indexOf(1), num2.indexOf(9));
} else {
...
const convertArry = "12345"; // if number is string then use split else first convert toString().split(')
const splitNum = convertArry.split("");
console.log("splitNum", splitNum);

console.log(splitNum.join("")); // 12345
...
console.log(reversed); // Output will be 123
